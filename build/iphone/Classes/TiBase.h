...
// in simulator we redefine to format for Telkom Developer console
...
{\
	NSString * exceptionName = [@"org.telkom." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.telkom";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UITelkomNativeItemNone = -1, 
	UITelkomNativeItemSpinner = -2,
	UITelkomNativeItemProgressBar = -3,
	
	UITelkomNativeItemSlider = -4,
	UITelkomNativeItemSwitch = -5,
	UITelkomNativeItemMultiButton = -6,
	UITelkomNativeItemSegmented = -7,
	
	UITelkomNativeItemTextView = -8,
	UITelkomNativeItemTextField = -9,
	UITelkomNativeItemSearchBar = -10,
	
	UITelkomNativeItemPicker = -11,
	UITelkomNativeItemDatePicker = -12,
	
	UITelkomNativeItemInfoLight = -13,
	UITelkomNativeItemInfoDark = -14,
	
	UITelkomNativeItemDisclosure = -15,
	
	UITelkomNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all Telkom instances of
